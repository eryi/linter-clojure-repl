...
export default LinterClojureRepl = window.LinterClojureRepl = {
  subscriptions: null,
  ...
  activate(state) {
    ...
    .subscribe(connectionNotification => {
      connectionNotification.do(conn => {
        if (conn) {
          ...
          , err => {
            ...
            atom.notifications.addError('Cannot load eastwood in REPL. Please add eastwood in your dev profile dependencies.');
      ...
      , () => {
        if (this.replConnection == false) {
          atom.notifications.addError("Cannot start REPL. Please try running 'lein repl' to see the issue.");
        }
        this.replConnection = false;
    ...
    return {
      ...
      lint: textEditor => {
        ...
        return new Promise( (resolve, reject) => {
          cljEval(this.replConnection, '(lint-filename-then-encode "' + editorPath + '"ref-opts)', 'linter-clojure-repl-parser')
          .subscribe(function(result) {
            ...
            atom.notifications.addError("Cannot lint " + editorPath + "\n" + err)
            resolve([])
          })
        })
      }
